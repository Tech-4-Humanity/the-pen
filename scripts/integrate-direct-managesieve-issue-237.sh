...
text = text.replace('disable `t4h-github-agent-routing`', "disable 't4h-github-agent-routing'")
...
! grep -Fq 'disable `t4h-github-agent-routing`' "$BUNDLE"

echo "STATUS=REAL"
echo "PATCH=DIRECT_MANAGESIEVE_SHELL_SAFE"
echo "BUNDLE=$BUNDLE"
echo "CLIENT=$CLIENT"
