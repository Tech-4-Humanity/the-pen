...
mkdir -p "$RECEIPT_DIR"
RECEIPT="$RECEIPT_DIR/ci-runtime-receipt.json"
SERVER_LOG="$RECEIPT_DIR/server.log"
...
insert into mode_definitions (mode_definition_id, version, name, purpose, definition, status)
values ('$MODE_ID', '1.0.0', 'Quiet Hours', 'CI validation mode', '{}'::jsonb, 'active');
SQL
...
HOUSEHOLD_RESPONSE="$(curl -fsS -X POST "http://127.0.0.1:$PORT/v1/households" \
  -H 'content-type: application/json' \
  -H "x-person-id: $OWNER_ID" \
  -H 'x-correlation-id: 22222222-2222-4222-8222-222222222222' \
  --data '{"name":"CI Household","timezone":"Australia/Sydney"}')"
HOUSEHOLD_ID="$(printf '%s' "$HOUSEHOLD_RESPONSE" | jq -r '.id')"
test -n "$HOUSEHOLD_ID"
...
MODE_RESPONSE="$(curl -fsS -X POST "http://127.0.0.1:$PORT/v1/households/$HOUSEHOLD_ID/modes" \
  ...
  --data "{\"modeDefinitionId\":\"$MODE_ID\",\"startsAt\":\"$STARTED_AT\"}")"
MODE_INSTANCE_ID="$(printf '%s' "$MODE_RESPONSE" | jq -r '.id')"
test -n "$MODE_INSTANCE_ID"
test "$MODE_INSTANCE_ID" != "null"

EVENT_COUNT="$(psql "$DATABASE_URL" -Atc "select count(*) from event_ledger where correlation_id in ('22222222-2222-4222-8222-222222222222','33333333-3333-4333-8333-333333333333');")"
test "$EVENT_COUNT" = "2"
...
SQL
ROLLBACK_AFTER="$(psql "$DATABASE_URL" -Atc "select count(*) from information_schema.tables where table_schema='public';")"
test "$ROLLBACK_BEFORE" -ge 1
test "$ROLLBACK_AFTER" = "0"
