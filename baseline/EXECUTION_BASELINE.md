...
**Established:** 2026-05-18 · derived from the Doolittles runtime build + its one failure.
...
2. **tool_search before declaring a limit.** A BLOCKED/PARTIAL classification citing a missing capability is **invalid** unless a logged tool_search returned nothing. Belief is not evidence; a null search result is.
3. **BUILD against verified shape.** Bind to real schema/columns/paths confirmed in step 1 — never guessed.
4. **PROVE by execution.** Run it. Capture the trace (exit code, output). No "should work."
...
- A later receipt may supersede an earlier one only by ADDING evidence, never by removing or softening a recorded failure.
- A remembered number/fact bound to an object must be re-verified against that object before use. (Origin of two logged failures: catalog_master '102 rows'; S2 '22 sellable'.)
...
- `receipts/doolittles/runtime/inbound/` — full two-way chain: BLOCKER→IMPLEMENTATION→RUNTIME→CLOSURE + FAILURE.
